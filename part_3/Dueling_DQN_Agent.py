...
from model import Dueling_Conv_Qnet
from collections import deque


class Dueling_DQN_Agent:
    def __init__(self):
        self.model = self.convert_to_cuda(Dueling_Conv_Qnet())
        self.target_model = self.convert_to_cuda(Dueling_Conv_Qnet())
    ...
    def plot(self, scores):
        ...
        plt.plot(scores)
        plt.title("Dueling DQN Super Mario")
        plt.ylim(ymin=0)
        plt.grid()
        plt.text(len(scores)-1, scores[-1], str(scores[-1]))
        plt.pause(.01)
        plt.savefig('Dueling DQN Agent Result.pdf')
    ...
    def get_action(self, state):
        if random.random() < self.epsilon:
            ...
            _, output = self.model(state.unsqueeze(0))
    ...
    def QTrainer(self):
        ...
        for iteration in range(self.number_of_iterations):
            ...
            values, Actions = self.model(state_batch)
            values_, Actions_ = self.target_model(new_state_batch)
            
            q_pred = T.add(values, (Actions - Actions.mean(dim=1, keepdim=True)))[self.batch_index, action_batch]

            q_new_states = T.add(values_, (Actions_ - Actions_.mean(dim=1, keepdim=True)))
            ...
            for i in range(self.batch_size):
                if terminated_batch[i]:
                    ...
                    q_target.append(reward_batch[i] + self.gamma*T.max(q_new_states[i]))
            ...
            if iteration % 100000 == 0:
                T.save(self.model, "pretrained_models/model_" + str(iteration) + "_iterations_dueling.pth")
