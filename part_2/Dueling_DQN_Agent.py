...
class Dueling_DQN_Agent:
    ...
    def plot(self, scores):
        ...
        plt.plot(scores)
        plt.title("Dueling DQN FlappyBird")
        plt.ylim(ymin=0)
        plt.grid()
        plt.text(len(scores)-1, scores[-1], str(scores[-1]))
        plt.pause(.01)
        plt.savefig('Dueling DQN Agent results.pdf')
    ...
    def QTrainer(self):
        ...
        for iteration in range(self.number_of_iterations):
            ...
            q_pred = T.add(values, (Actions - Actions.mean(dim=1, keepdim=True)))
            ...
            for i in range(len(state_batch)):
                if terminated_batch[i]:
                    ...
                    q_target += reward_batch[i] + self.gamma*T.max(q_new_states[i])
