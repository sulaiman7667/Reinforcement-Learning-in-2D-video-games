...
if __name__ == "__main__":
    ...
    if mode == 'test':
        ...
        # select agent
        agent_1 = DQN_Agent(game_env)
        agent_2 = Double_DQN_Agent(game_env)
        agent_3 = Dueling_DQN_Agent(game_env)
        ...
        max_score, avg_score = agent_1.QTest(model_1)
        print("Agent: DQN", "Max Score:", max_score, "Average Score:", avg_score)

        max_score, avg_score = agent_2.QTest(model_2)
        print("Agent: Double DQN", "Max Score:", max_score, "Average Score:", avg_score)

        max_score, avg_score = agent_3.QTest(model_3)
        print("Agent: Dueling DQN", "Max Score:", max_score, "Average Score:", avg_score)

    elif mode == 'train':
        if not os.path.exists('pretrained_models/'):
            os.mkdir('pretrained_models/')
            
        agent = DQN_Agent(game_env)
        agent.QTrainer()
        scores_1 = agent.scores

        agent = Double_DQN_Agent(game_env)
        agent.QTrainer()
        scores_2 = agent.scores

        agent = Dueling_DQN_Agent(game_env)
        agent.QTrainer()
